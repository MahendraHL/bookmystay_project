		...
		public List<BookingDto> fetchBookingDetails(Integer customerId) {
			...
			if(!booking.isEmpty()) {
				return booking.stream()
						.map(bookings -> BookingDto.builder().bookingId(bookings.getBookingId())
								.email(bookings.getEmail()).bookingId(bookings.getBookingId())
								.checkInDate(bookings.getCheckInDate()).checkOutDate(bookings.getCheckOutDate())
								.hotelName(bookings.getHotelName()).noOfPerson(bookings.getNoOfPerson()).build())
